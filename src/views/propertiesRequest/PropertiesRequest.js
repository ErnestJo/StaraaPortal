      ...
      axios
        ...
        .then((data) => {
          ...
          if ((data.data[0].data[0].code = 111)) {
            props.onHide()
            Notify.notifySuccess('Request was added Made')
            props.reloadPage()
          } else {
            Notify.notifyError('failed to add new Request')
          }
        })
      ...
      <Card>
        ...
        <Card.Body>
          <DataGrid
            ...
            dataSource={dataPropertiesReqs.filter(
              (request) =>
                request.request_status.includes('Pending', 'Not Returned') ||
                request.request_status.includes('Not Redurned'),
            )}
            showRowLines={true}
            showBorders={true}
          >
              ...
              cellRender={(e) => (
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  <Button
                    ...
                    className="mx-1"
                    onClick={() => {
                      console.log(e.data.request_status)
                      if (e.data.request_status == 'Pending') {
                        var body = {
                          properties_req_id: e.data.properties_req_id,
                          request_status: 5,
                        }
                        console.log(body)
                        axios
                          .post(UpdatePropertiesReqStatus_URL, body, {
                            headers: { 'content-Type': 'application/json' },
                          })
                          .then((data) => {
                            console.log(data.data[0].data[0])
                            if ((data.data[0].data[0].code = 111)) {
                              fetchData()
                              Notify.notifySuccess('Property is Booked')
                            } else {
                              Notify.notifyError('Property was not able to book')
                            }
                          })
                      }
                    }}
                  ...
                  <Button
                    ...
                    onClick={() => {
                      console.log(e.data.request_status)
                      if (e.data.request_status == 'Not Redurned') {
                        var body = {
                          properties_req_id: e.data.properties_req_id,
                          request_status: 4,
                        }
                        ...
                        axios
                          ...
                          .then((data) => {
                            ...
                            if ((data.data[0].data[0].code = 111)) {
                              fetchData()
                              Notify.notifySuccess('Returned added Succefully')
                            } else {
                              Notify.notifyError('Returned was not done')
                            }
                          })
                      }
                    }}
